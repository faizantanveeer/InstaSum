...
class SummarizationService:
    ...
    def generate_title(self, transcript: str, caption: str = "") -> str:
        ...
        if self.client is None and not self.api_key and self.azure_client is None:
            return self._fallback_title(transcript, caption)

        system = (
            "Generate a concise, informative title for an Instagram Reel based on caption and transcript. "
            "Return JSON only: {\"title\":\"...\"}. "
            "Keep it natural, specific, and clear. No emojis. No hashtags."
        )
        user = (
            "Caption:\n"
            f"{caption or 'No caption available'}\n\n"
            "Transcript:\n"
            f"{transcript or 'No spoken transcript available'}"
        )
        ...
        if not content and self.api_key and self.azure_client is None:
            content = self._direct_openai_title(system, user)
        ...
        if content:
            ...
            # Non-JSON fallback from model response.
            title_line = content.splitlines()[0].strip().strip('"')
            if title_line:
                return title_line[:140]
    ...
    def _direct_openai_title(self, system: str, user: str) -> str:
        try:
            resp = requests.post(
    ...
    def _fallback_title(self, transcript: str, caption: str) -> str:
        cap = (caption or "").strip()
        if cap:
            cap = re.sub(r"#[A-Za-z0-9_]+", "", cap)
            cap = re.sub(r"\s+", " ", cap).strip()
            if cap:
                return " ".join(cap.split()[:10])[:140]

        tr = (transcript or "").strip()
        if tr:
            first_sentence = re.split(r"[.!?]\s+", tr, maxsplit=1)[0].strip()
            if first_sentence:
                return " ".join(first_sentence.split()[:10])[:140]
